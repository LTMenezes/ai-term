    ...
    if user_input == "y":
        try:
            with tempfile.NamedTemporaryFile(mode="w+") as temp:
                ...
                temp.flush()
                result = subprocess.run(f"bash {temp.name}", shell=True, check=True, capture_output=True, text=True)
            output_lines = result.stdout.strip().split("\n")
            current_directory = output_lines[-1] if output_lines[-1] is not None and output_lines[-1].strip() else current_directory
